          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {!coursesLoading && coursesOptions.length === 0 && (
                <p className="text-sm text-muted-foreground">No se encontraron grupos para esta materia.</p>
              )}
              {coursesOptions.map((c, i) => {
                ...
                // Convertir CourseResponse a MateriaExtraida para el store
                ...
                const checkAdd = isSelected ? { allowed: true } : canAddMateria(materiaFormat, selectedMaterias)
                ...
                return (
                  <button
                    ...
                    title={!checkAdd.allowed && !isSelected ? checkAdd.reason : undefined}
                    className={cn(
                  ...
                  >
                    ...
                    {c.time_slots.length > 0 ? (
                      <div className="mt-1 text-xs text-muted-foreground">
                        {c.time_slots.map(ts => formatHorario(ts.day, ts.start_time.substring(0, 5), ts.end_time.substring(0, 5), ts.classroom)).join(" | ")}
                      </div>
                    ) : (
                      <div className="mt-1 text-xs text-muted-foreground">Sin horario asignado</div>
                    )}
