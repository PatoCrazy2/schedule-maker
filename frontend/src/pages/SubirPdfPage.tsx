      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          <h3 className="mb-2 text-sm font-semibold flex items-center gap-2">
            ...
            {file ? "Vista previa del PDF" : "Sin archivo nuevo"}
          </h3>
          {pdfUrl ? (
            <PdfViewer src={pdfUrl} filename={file?.name} />
          ) : (
            <div className="w-full h-[500px] bg-muted/30 border border-dashed rounded-lg flex items-center justify-center text-sm text-muted-foreground p-6 text-center">
              {oferta ? "Los datos se cargaron desde el servidor. No hay vista previa del PDF original disponible." : "No se ha seleccionado o cargado ningún PDF para su previsualización."}
            </div>
          )}
        </div>
        <div>
